"""
Reasoning Internalization Test: Do CoTFT models learn better representations,
or just better output format?

Evaluates CoTFT models using the IoFT prompt (direct translation, no reasoning)
and compares to the actual IoFT model.

If CoTFT-direct > IoFT → reasoning is internalized in weights
...
# All models use the DIRECT (IoFT-style) prompt — no reasoning allowed
MODELS = [
    ("IoFT (direct prompt)", "./checkpoints_v1/ioft/final"),
    ("CoTFT-CoT (direct prompt)", "./checkpoints_v1/cotft_cot/final"),
    ("CoTFT-MAPS (direct prompt)", "./checkpoints_v1/cotft_maps/final"),
]


def extract_translation(text):
    """Same extraction logic as eval_all_ft.py for fair comparison."""
    text = text.strip()
...
def translate(model, tokenizer, sources, max_new_tokens=256, batch_size=8):
    """Translate using DIRECT prompt only — no reasoning scaffolding."""
    tokenizer.padding_side = 'left'
    translations = []
    for i in range(0, len(sources), batch_size):
        batch = sources[i:i + batch_size]
        # IoFT-style prompt: direct translation, no thinking
...
def main():
    ...
    for label, ckpt_dir in MODELS:
        ...
        print("Translating (direct prompt, no reasoning)...")
    ...
    # Summary
    print(f"\n{'='*60}")
    print("INTERNALIZATION TEST: CoTFT models with direct (IoFT) prompt")
    print(f"{'='*60}")
    print(f"{'Model':<30} {'BLEU':>8} {'chrF++':>8} {'COMET':>8}")
    print("-" * 56)
    for r in all_results:
        print(f"{r['label']:<30} {r['bleu']:>8.2f} {r['chrf']:>8.2f} {r['comet']:>8.4f}")

    print(f"\nReference (CoTFT with reasoning at inference):")
    print(f"  IoFT:          BLEU 6.52, chrF++ 31.29, COMET 0.6484")
    print(f"  CoTFT (CoT):   BLEU 4.32, chrF++ 28.47, COMET 0.6190")
    print(f"  CoTFT (MAPS):  BLEU 6.99, chrF++ 33.56, COMET 0.6655")

    with open("results_internalization.json", "w") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)
    print(f"\nSaved to results_internalization.json")
